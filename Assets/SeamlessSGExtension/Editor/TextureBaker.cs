...
namespace Seamless.SGExtension
{
    static internal class TextureBaker
    {
        ...
        static internal void Bake(ExportNode node)
        {
            ...
            {
                string absPath = EditorUtility.SaveFilePanel("Save Path", path.Substring(0, path.IndexOf(path.Split('/')[path.Split('/').Length - 1])), path.Split('/')[path.Split('/').Length - 1], "png");
        ...
        static Texture2D GetPreviewTexture(Material material, MaterialPropertyBlock materialPropertyBlock, int width, int height, bool mipChain, TextureType textureType)
        {
            ...
            Graphics.DrawMesh(quadMesh, Matrix4x4.identity, material, 1, cam, 0, materialPropertyBlock, ShadowCastingMode.Off, false, null, false);
            cam.Render();
